...
def scrape_gold():
    ...
    try:
        ...
        for row in rows:
            ...
            if len(cols) == 5: # Hàng có cột brand
                product_name, buy, sell = cols[1], cols[3], cols[4]
            elif len(cols) == 4: # Hàng bị ẩn cột brand
                product_name, buy, sell = cols[0], cols[2], cols[3]
            else: continue
            ...
            if re.sub(r'[^\d]', '', buy + sell):
                ...
                data.append({"product_id": re.sub(r'_+', '_', product_id), "asset_type": "Vàng", "product_name": product_name, "unit": "Chỉ", "buy": buy, "sell": sell, "currency": "VND"})
